...
class LaunchkeyCommonControlSurface(ControlSurface):
    ...
    def setup(self):
        _logger.debug("ENTER LaunchkeyCommonControlSurface.setup()")
        try:
            _logger.debug("Calling super().setup()")
            super().setup()
            _logger.debug("super().setup() completed")

            _logger.debug("Binding __on_main_pad_mode_changed.subject to component_map['Main_Pad_Modes']")
            self.__on_main_pad_mode_changed.subject = self.component_map["Main_Pad_Modes"]

            _logger.debug("Registering slots for Daw_Pad_Modes.selected_mode and Sequencer_Modes.selected_mode")
            ...
            _logger.debug("Setting Keyboard.note_editor from Step_Sequence.note_editor")
            self.component_map["Keyboard"].set_note_editor(self.component_map["Step_Sequence"].note_editor)

            _logger.debug("LaunchkeyCommonControlSurface.setup() completed")
        except Exception:
